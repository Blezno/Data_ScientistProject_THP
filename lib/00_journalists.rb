...
#Question 3
def fivecarac(arr)
    print "Devine combien handle contiennent juste 5 caractères...Tu ne trouves pas ? "
    arr.count {|i| i.length == 6 }
end

#Question 4
def maj(arr)
    print "Combien commencent par une majuscule (première lettre juste après le @) ? "
    arr.count {|i| i =~ /^@[A-Z]/}
end
...
#Question 6
def sizehandle(arr)
    print "Trie la liste de handle par taille des handle (les plus petits en premiers, les plus grands après) "
    arr.sort_by(&:length)
end

#Question 7
def position(arr)
    print "Quelle est la position dans l'array de la personne @epenser ? "
    arr.index('@epenser')
end
...
def by_size(arr)
    handle = []
    28.times do |i|
    handle << arr.count{|j| j.length == i}
    end
    28.times do |i|
        puts "Il y a #{handle[i]} mots avec #{i -1} lettres " if handle [i] != 0
    end
end

def perform(arr)
    puts "------ Question 1 ------"
    puts how_many_handle(arr)
    puts "------ Question 2 ------"
    puts shortest_jour(arr)
    puts "------ Question 3 ------"
    puts fivecarac(arr)
    puts "------ Question 4 ------"
    puts maj(arr)
